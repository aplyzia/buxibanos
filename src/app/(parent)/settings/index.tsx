...
export default function ParentSettingsScreen() {
  ...
  const parentName = profile && "full_name" in profile ? profile.full_name : "";
  const parentId = profile && "id" in profile ? profile.id : "";
  const parentPhone = profile && "phone" in profile ? (profile as any).phone : null;
  const currentPref =
  ...
  const handleNotifChange = async (pref: NotifPref) => {
    if (pref === notifPref || !parentId) return;
    setNotifPref(pref);
    setIsSaving(true);
      ...
      <View className="pt-14 pb-3 px-4 flex-row items-center">
        <Pressable
          onPress={() =>
            router.canGoBack()
              ? router.back()
              : router.replace("/(parent)/(tabs)/messages" as any)
          }
