...
export default function TasksScreen() {
  ...
  // profile.id = staff table PK (what completed_by REFERENCES staff(id) needs)
    ...
    <GlassBackground variant="staff">
      ...
      {isLoading ? (
        ...
        <View className="flex-1 items-center justify-center px-8">
          ...
          <Pressable
            onPress={() => organizationId && fetchTasks(organizationId)}
        ...
        <FlatList
          ...
          onRefresh={() => organizationId && fetchTasks(organizationId)}
          renderItem={({ item }) => (
            <TaskRow
              task={item}
      ...
      {/* FAB — create manual task */}
      <Pressable
        onPress={() => router.push("/(staff)/tasks/create" as never)}
  ...
  return (
    <View className="mx-4 mb-3">
      <GlassCard className="p-4">
        ...
        {/* Completion info — who marked it done/dismissed */}
        ...
        {task.source_message_id && (
          <Pressable
            onPress={() => onNavigateToMessage(task.source_message_id!)}
            className="mb-2"
          >
            <Text
              className="text-xs"
              style={{ color: colors.accentColor, textDecorationLine: "underline" }}
            >
              {t("tasks.viewSourceMessage")}
            </Text>
          </Pressable>
        )}

        {/* Bottom row: assigned + actions */}
        <View className="flex-row items-center justify-between">
          <Text className="text-xs" style={{ color: colors.textMuted }}>
            {task.assigned_to
