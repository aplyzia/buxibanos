...
export default function TeacherChannelDetailScreen() {
  ...
  // ─── Attendance detection on teacher messages ───
  const runAttendanceDetection = async (content: string) => {
    if (!organizationId || !staffId) return;
    try {
      const result = await detectAttendance(content);
      if (!result.detected || !result.status) return;

      // Try to extract a student name from the message (simple heuristic)
      // The detectAttendance function handles the analysis
      // We log it but don't create attendance without a student match
      console.log("[Teacher STT] Attendance detected:", result);

      // Note: Full attendance record creation requires matching student name
      // from the message text. For now, log the detection.
      // In the future, the n8n backend will handle this more robustly.
    } catch {
  ...
  const handleSendText = async (text: string) => {
    ...
    // Run attendance detection in background
    runAttendanceDetection(text);
  };
