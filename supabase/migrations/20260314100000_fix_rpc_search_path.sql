-- Fix SET search_path = public on all WF2–WF6 RPC functions
-- Without this, SECURITY DEFINER functions silently return null when querying tables
...
begin
  if p_target_type = 'all' then
    return (select json_agg(json_build_object('push_token', push_token))
            from public.parents
            where organization_id = p_organization_id and push_token is not null);
  else
    return (select json_agg(json_build_object('push_token', push_token))
            from public.parents
            where organization_id = p_organization_id
...
begin
  return (
    select json_agg(json_build_object(
      ...
      'push_token', push_token, 'voip_push_token', null
    ))
    from public.staff
    where organization_id = p_organization_id
...
begin
  return (
    select json_agg(json_build_object(
      'id', id, 'full_name', full_name, 'push_token', push_token
    ))
    from public.staff
    where organization_id = p_organization_id
      and is_active = true and push_token is not null
